...
void FPM10A_Receive_Data(unsigned char ucLength) {
	...
	while (avaliable() < ucLength);

	for (i = 0;i < ucLength;i++)
		FPM10A_RECEICE_BUFFER[i] = readch();
...
uint32_t FPM10A_DeleChar(uint16_t id, uint16_t n) {
	...
	if (FPM10A_RECEICE_BUFFER[9] == 0) {//如果删除成功
		return 1;
	}
	else {
		return 0;
...
void FINGERPRINT_Cmd_Img_To_Buffer1(void) {
	...
	{
		_put_char(FPM10A_Img_To_Buffer1[i]);
	}
}
...
void FINGERPRINT_Cmd_Img_To_Buffer2(void) {
	...
	for (i = 0;i < 7;i++)   //发送命令 将图像转换成 特征码 存放在 CHAR_buffer1
	{
		_put_char(FPM10A_Img_To_Buffer2[i]);
	}
}
//搜索全部用户999枚
...
void FINGERPRINT_Cmd_Delete_All_Model(void) {
	...
	{
		_put_char(FPM10A_Delete_All_Model[i]);
	}
}
//保存指纹
void FPM10A_Cmd_Save_Finger(unsigned int storeID) {
	...
	FPM10A_Cmd_Send_Pack_Head(); //发送通信协议包头	
	for (i = 0;i < 9;i++)
		_put_char(FPM10A_Save_Finger[i]);      //发送命令 将图像转换成 特征码 存放在 CHAR_buffer1
}
...
uint32_t Device_Check(void) {
	...
	// }
	return (FPM10A_RECEICE_BUFFER[9] == 0);
}
